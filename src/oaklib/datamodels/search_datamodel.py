...
# Generation date: 2022-12-01T10:19:21
...
class ComplexQuery(YAMLRoot):
    ...
    all_of: Optional[Union[Union[dict, "ComplexQuery"], List[Union[dict, "ComplexQuery"]]]] = empty_list()
    any_of: Optional[Union[Union[dict, "ComplexQuery"], List[Union[dict, "ComplexQuery"]]]] = empty_list()
    none_of: Optional[Union[Union[dict, "ComplexQuery"], List[Union[dict, "ComplexQuery"]]]] = empty_list()
    ...
    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        ...
        self.all_of = [v if isinstance(v, ComplexQuery) else ComplexQuery(**as_dict(v)) for v in self.all_of]

        if not isinstance(self.any_of, list):
            self.any_of = [self.any_of] if self.any_of is not None else []
        self.any_of = [v if isinstance(v, ComplexQuery) else ComplexQuery(**as_dict(v)) for v in self.any_of]

        if not isinstance(self.none_of, list):
            self.none_of = [self.none_of] if self.none_of is not None else []
        self.none_of = [v if isinstance(v, ComplexQuery) else ComplexQuery(**as_dict(v)) for v in self.none_of]
...
class SearchProperty(EnumDefinitionImpl):
    ...
    REPLACEMENT_IDENTIFIER = PermissibleValue(text="REPLACEMENT_IDENTIFIER",
                                                                   description="A replacement identifier or URI for the entity",
                                                                   meaning=SCHEMA.identifier)
    MAPPED_IDENTIFIER = PermissibleValue(text="MAPPED_IDENTIFIER",
...
slots.searchBaseConfiguration__is_fuzzy = Slot(uri=SEARCH.is_fuzzy, name="searchBaseConfiguration__is_fuzzy", curie=SEARCH.curie('is_fuzzy'),
                   model_uri=SEARCH.searchBaseConfiguration__is_fuzzy, domain=None, range=Optional[Union[bool, Bool]])
...
slots.searchBaseConfiguration__categories = Slot(uri=SEARCH.categories, name="searchBaseConfiguration__categories", curie=SEARCH.curie('categories'),
                   model_uri=SEARCH.searchBaseConfiguration__categories, domain=None, range=Optional[Union[Union[str, URIorCURIE], List[Union[str, URIorCURIE]]]])

slots.complexQuery__all_of = Slot(uri=SEARCH.all_of, name="complexQuery__all_of", curie=SEARCH.curie('all_of'),
                   model_uri=SEARCH.complexQuery__all_of, domain=None, range=Optional[Union[Union[dict, ComplexQuery], List[Union[dict, ComplexQuery]]]])

slots.complexQuery__any_of = Slot(uri=SEARCH.any_of, name="complexQuery__any_of", curie=SEARCH.curie('any_of'),
                   model_uri=SEARCH.complexQuery__any_of, domain=None, range=Optional[Union[Union[dict, ComplexQuery], List[Union[dict, ComplexQuery]]]])

slots.complexQuery__none_of = Slot(uri=SEARCH.none_of, name="complexQuery__none_of", curie=SEARCH.curie('none_of'),
                   model_uri=SEARCH.complexQuery__none_of, domain=None, range=Optional[Union[Union[dict, ComplexQuery], List[Union[dict, ComplexQuery]]]])

slots.complexQuery__path_to = Slot(uri=SEARCH.path_to, name="complexQuery__path_to", curie=SEARCH.curie('path_to'),
                   model_uri=SEARCH.complexQuery__path_to, domain=None, range=Optional[str])

slots.complexQuery__atom = Slot(uri=SEARCH.atom, name="complexQuery__atom", curie=SEARCH.curie('atom'),
                   model_uri=SEARCH.complexQuery__atom, domain=None, range=Optional[Union[dict, SearchBaseConfiguration]])

slots.pathExpression__traversal = Slot(uri=SEARCH.traversal, name="pathExpression__traversal", curie=SEARCH.curie('traversal'),
                   model_uri=SEARCH.pathExpression__traversal, domain=None, range=Optional[Union[str, "GraphFunction"]])

slots.pathExpression__graph_predicates = Slot(uri=SEARCH.graph_predicates, name="pathExpression__graph_predicates", curie=SEARCH.curie('graph_predicates'),
                   model_uri=SEARCH.pathExpression__graph_predicates, domain=None, range=Optional[Union[Union[str, URIorCURIE], List[Union[str, URIorCURIE]]]])

slots.pathExpression__search_term = Slot(uri=SEARCH.search_term, name="pathExpression__search_term", curie=SEARCH.curie('search_term'),
                   model_uri=SEARCH.pathExpression__search_term, domain=None, range=Optional[Union[dict, SearchBaseConfiguration]])
