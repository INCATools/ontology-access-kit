...
import click
from semsql.sqla.semsql import Statements, ObjectPropertyNode

from oaklib import BasicOntologyInterface, get_adapter
from oaklib.datamodels.vocabulary import RDFS_COMMENT, HAS_DEFINITION_CURIE, SKOS_DEFINITION_CURIE, DESCRIPTION, \
    SUBCLASS_OF, OWL_SOME_VALUES_FROM, EQUIVALENT_CLASS
...
def calculate_ontology_text_content(adapter: SqlImplementation, language="en", annotation_predicates: Optional[List[PRED_CURIE]] = None) -> float:
    """
    ...
    logger.info(f"Calculating text content for {adapter.implementation_name}, language={language}, annotation_predicates={annotation_predicates}")
    for row in session.query(Statements).all():
        ...
        if v:
            ...
            if p not in [SUBCLASS_OF, OWL_SOME_VALUES_FROM, EQUIVALENT_CLASS] and p not in object_properties:
                continue
...
@click.option("-A", "--annotation-predicate", multiple=False, help="Predicates to use (comma-separated)")
@click.option("--default-annotations/--no-default-annotations", default=True, help="Use default annotations")
@click.argument("inputs", nargs=-1)
def main(inputs: List[str], verbose: int, language: str, annotation_predicate: str, default_annotations: bool):
    """
    Calculate the text content of an ontology.
    """
    ...
    for input_name in inputs:
        try:
            ...
            tc = calculate_ontology_text_content(adapter, language=language, annotation_predicates=annotation_predicates)
