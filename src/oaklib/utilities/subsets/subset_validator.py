...
def validate_subset(adapter: SemanticSimilarityInterface, configuration: Optional[SubsetValidationConfig]=None) -> SubsetValidationResult:
    """
    ...
    if configuration.ic_score_adapter_name:
        ...
        if not isinstance(ic_adapter, SemanticSimilarityInterface):
            raise ValueError("Information content adapter must implement SemanticSimilarityInterface")
    ...
    result = SubsetValidationResult(subset_name=configuration.subset_name,
                                    configuration=configuration,
                                    terms_used=terms, ignored_terms=[t for t in configuration.subset_terms if t not in terms])
    if len(terms) < 2:
        result.combined_score = 0.0
        return result
    non_leaf_terms = set()
    for i, term_i in enumerate(terms):
        term_i_ancs = set(graph_adapter.ancestors(term_i, configuration.predicates, reflexive=False))
    ...
    overall_score = 0.0
    n = 0
    for cn, df in [("overall", df_all), ("sibling_pairs", df_sibs), ("ancestor_pairs", df_ancs), ("leaf_pairs", df_leafs)]:
        min_ic_dist = df['ic_distance'].min()
        min_j = df['j'].min()
        # Get witnesses for min_ic_dist
        min_ic_dist_rows = df[np.isclose(df['ic_distance'], min_ic_dist)]
        min_ic_dist_pairs = []
        for i, row in min_ic_dist_rows.iterrows():
        ...
        min_j_pairs = []
        for i, row in min_j_rows.iterrows():
